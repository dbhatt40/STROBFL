...
def mal_data_create(X_test, Y_test, Y_test_uncat):
	...
	if args.mal_obj == 'all':
		...
		print("Initial class for single attack: %s" % Y_test_uncat[r])
		...
		print("Target class for single attack: %s" % mal_data_Y[0])
	elif 'multiple' in args.mal_obj:
		print('Multiple target attack with num targets:', args.mal_num)
		target_indices = np.random.choice(len(X_test), args.mal_num)
		mal_data_X = X_test[target_indices]
		print("Initial classes for multiple attack: %s" % Y_test_uncat[target_indices])
		...
		print("Target classes for multiple attack: %s" % mal_data_Y)
...
def mal_data_setup(X_test, Y_test, Y_test_uncat, gen_flag=True):
	...
	if not os.path.exists('data/mal_X_%s_%s.npy' % (args.dataset, args.mal_obj)):
		...
		print("Initial classes before attack: %s" % true_labels)
		print("Target classes after attack: %s" % mal_data_Y)
